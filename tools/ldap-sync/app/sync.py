...
def run_sync(settings: Settings) -> dict[str, int]:
    ...
    # Optionally create users that are in LDAP but not in n8n.
    # In trusted-login deployments this should usually remain disabled.
    if settings.ldap_sync_create_users:
        invitations: list[dict[str, str]] = [
            {"email": u["email"], "role": u["role"]}
            for u in ldap_users
            if u["email"] not in n8n_by_email
        ]
